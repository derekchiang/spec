...
* **pathBlacklist** (list of strings, optional) blacklist of absolute paths that will be removed in the app's rootfs after rendering. This must be a complete and absolute set. An empty list is equivalent to an absent value and means that all files in this image and any dependencies will be available in the rootfs.  Importantly, a manifest can have **either a non-empty whitelist or a non-empty blacklist, but not both**.
